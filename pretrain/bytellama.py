...
def setup(
    devices: int = 8,
    data_dir: Path = Path("data/redpajama_sample"),
    ...
    fabric.launch(main, data_dir, resume)
    # main(fabric, data_dir, resume)


def main(fabric, data_dir, resume):
    ...
    train_data, val_data = load_datasets(data_dir, block_size=config.block_size)
...
class Dataset(IterableDataset):
    ...
    def __iter__(self):
        ...
        while True:
            ...
            x = torch.from_numpy((data[i : i + self.block_size + 1]).astype(np.int64))
            yield x
